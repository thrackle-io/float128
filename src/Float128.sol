...
library Float128 {
    ...
    // ln10, ln2 and ln1.1 represented as integers with 70 decimal places
    int constant ln10_M =  23025850929940456840179914546843642076011014886287729760333279009675726;
    int constant ln2_M =    6931471805599453094172321214581765680755001343602552541206800094933936;
    int constant ln1dot1_M = 953101798043248600439521232807650922206053653086441991852398081630010;
    int constant ln1dot01_M = 99503308531680828482153575442607416886796099400587978646095597668666;
    int constant ln1dot001_M = 9995003330835331668093989205350114607550623931665519970196668289003;

    // ln10, ln2 and ln1.1 represented as integers with M significant digits
    // int ln10_M = ln10_70; // 10**(70-M+1)
    // int ln2_M = ln2_70; // 10**(70-M)
    // int ln1dot1_M = ln1dot1_70; // 10**(70-M-1)
    // int ln1dot01_M = ln1dot01_70; // 10**(70-M-2)
    // int ln1dot001_M = ln1dot001_70; // 10**(70-M-3)

    // ln10, ln2 and ln1.1 represented as float128
    packedFloat constant ln10 = packedFloat.wrap(57631247817450639466916399678377984585681629858437802053308154406552768610828);
    packedFloat constant ln2 = packedFloat.wrap(57617575929603672933020748731348030977702325377248294479599698904161824288960);
    packedFloat constant ln1dot1 = packedFloat.wrap(57603701107702929161675104640453437103929836059592015714672457772623619947152);
    packedFloat constant ln1dot01 = packedFloat.wrap(57589608262471975646722820274969112179049878095757930200802093480908335638944);
    packedFloat constant ln1dot001 = packedFloat.wrap(57575477953201515296968141758129277768215145325982085921347679342150208320736);
    ...
    function findNumberOfDigits(uint x) internal pure returns (uint log) {
        ...
        function ln_prec(int mantissa, int exp) public pure returns (packedFloat result) {
            ...
            if(exp <= 0 && mantissa == comparison) {
                ...
                // assert 76 <= m10 and m10 <= 77 # This assertion should hold. Included for testing purposes.
                ...
                // assert exp_one_over_argument + m10 <= 0 # Shouldn't be needed. It has been included for testing purposes.
            ...
            if(len_mantissa <= positiveExp) {
                ...
                console2.log("q1", q1);
                console2.log("q1 mantissa:", uMantissa);
                ...
                console2.log("q2", q2);
                console2.log("q2 mantissa:", uMantissa);
                ...
                console2.log("q3", q3);
                console2.log("q3 mantissa:", uMantissa);
                ...
                packedFloat firstTerm = add(result, toPackedFloat(k, 0));
                packedFloat secondTerm = mul(firstTerm, ln2);
                packedFloat thirdTerm = add(secondTerm, toPackedFloat(q1, 0));
                packedFloat fourthTerm = mul(thirdTerm, ln1dot1);
                packedFloat fifthTerm = add(fourthTerm, toPackedFloat(q2, 0));
                packedFloat sixthTerm = mul(fifthTerm, lnB);
                packedFloat seventhTerm = add(sixthTerm, toPackedFloat(q3, 0));
                packedFloat eigthTerm = mul(seventhTerm, lnC);
                packedFloat ninthTerm = sub(eigthTerm, toPackedFloat(m10, 0));
                packedFloat tenthTerm = mul(ninthTerm, ln10);
                result = mul(tenthTerm, toPackedFloat(-1, 0));
            }
        }
